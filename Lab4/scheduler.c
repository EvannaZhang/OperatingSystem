...
void updateall()
{
	...
	for (p = head; p != NULL; p = p->next) {
        p->job->wait_time += 100;
        // Modified, increase the current priority if some job wait too long
        // Maximun of curpri is 3
...
struct waitqueue* jobselect()
{
	...
	if (head) {
		for (prev = head, p = head; p != NULL; prev = p, p = p->next) {
            ...
            // Modified
            // Select the one with the highest wait time
        ...
        if (selectprev == select) {
             // Modified, cannot directly assign null to head
            head = head -> next;
...
void jobswitch()
{
	...
	if (next == NULL && current == NULL)          /* no job to run */
        ...
        // Modified, should compare current and next
...
void do_deq(struct jobcmd deqcmd)
{
	...
	if (current && current->job->jid == deqid) {
		...
		if (head) {
            ...
            if(select){
                if (selectprev == select) {
                    // if head selected, update select
                    head = head -> next;
                }else{
                    selectprev->next = select->next;
                }
...
void do_stat()
{
	...
	*/
	...
	if (current) {
        ...
        // added default priority and current priority to show priority based RR
